            ...
            <div className="card border-secondary mb-3" key={this.state.product.id}>
                ...
                <div className="card-body">
                    ...
                    <p className="card-text">Price :
                        $ {(this.state.product.price == null) ? "No price" : this.state.product.price}</p>
                    <p className="card-text">Quantity available
                        : {(this.state.product.quantity <= 0) ? "Out of stock!" : this.state.product.quantity}</p>
                    <p className="card-text"><img src={this.state.product.photo} alt={this.state.product.name}/></p>

                    <button type="button" className="btn btn-primary"
                            onClick={() => this.props.buyProduct(this.state.product)}>BUY
                    </button>
                    <button type="button" className="btn btn-warning"
                            onClick={() => this.props.removeProduct(this.state.product)}>DELETE
                    </button>
